...
# # s3_prefix = 'clinical/manual_files/'
variance = int(Variable.get("koios_time_delta_dev"))

# # s3_prefix = 'clinical/manual_files/'
s3_prefix = Variable.get("koios_s3_prefix")

# s3_prefix = 'clinical/4g/XL495-10/'
...
def process_table(matching_items, last_modified_pst):
    ...
    for matching_item in matching_items:
        ...
        processed_list = []
        if delimiter == 'Excel' and matching_item.get('multi_sheet_tables'):
            for sheet_name, table_info in matching_item['multi_sheet_tables'].items():
                processed_list.append({'table_name': table_info['table_name'], 
                                       'sheet_name': sheet_name,
                                       'expected_columns': table_info.get('expected_columns', None)}) 
        else:
            processed_list.append({'table_name': table_name,
                                   'sheet_name': None,
                                   'expected_columns': matching_item.get('expected_columns', None)})
        for process_item in processed_list: 
            read_s3.load_data_to_rds(secret_name,s3_key,process_item['table_name'], 
                                     delimiter=delimiter,
                                     schema_name=schema_name,
                                     last_modified_pst=last_modified_pst,
                                     study_id=study_id,
                                     sheet_name=process_item['sheet_name'],
                                     expected_columns=process_item['expected_columns'])
            logging.info (f"Loading completed for {process_item['table_name']}")    
        #---------New Code Ends Here-----------------        
            # # Check if multi-sheet tables are defined in config
            # multi_sheet_tables = matching_item.get('multi_sheet_tables')
            # # print(f"Multi Sheet Tables: {multi_sheet_tables}")
            # if multi_sheet_tables:
            #     for sheet_name, table_info in multi_sheet_tables.items():
            #         table_name = table_info['table_name']
            #         # logging.info(f'Loading sheet "{sheet_name}" to table: {table_name}')
            #         expected_columns = table_info.get('expected_columns', None)
            #         # logging.info(f'expected_columns for {table_name}: {expected_columns}')
            #         read_s3.load_data_to_rds(
            #             secret_name,s3_key,table_name,
            #             delimiter=delimiter,
            #             schema_name=schema_name,
            #             last_modified_pst=last_modified_pst,
            #             study_id=matching_item.get('studyid'),
            ...
            #             )   
            #     continue  # skip rest of loop for this multi-sheet file
            # else:
            #     table_name = matching_item['table_name']
            #     expected_columns= matching_item.get('expected_columns', None)
            #     # logging.info(f'expected_columns for {table_name}: {expected_columns}')
        ...
        # else:
        #     table_name = matching_item['table_name']
        file_processed_list.append(s3_key)
        
        # elif matching_item['delimiter'] == 'Excel':
        #     logging.info(f"Processing Excel: {s3_key}")
        #     logging.info(f'Loading data from file: {s3_key} /n Loading data to table: {table_name}')
        # read_s3.load_data_to_rds(secret_name,s3_key,table_name,
        #                                     delimiter=delimiter,schema_name=schema_name, last_modified_pst=last_modified_pst, study_id = study_id, sheet_name=sheet_name, 
        #                 expected_columns=expected_columns)
    ...
    for config in config_data:
        if config['file_pattern'] in s3_obj and config['studyid'] in s3_obj:
            matching_config.append({"table_name" :config['global_filename'], 
                                    ...
                                    "multi_sheet_tables": config.get('multi_sheet_tables')})
