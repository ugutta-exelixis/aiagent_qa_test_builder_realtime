...
with open('/usr/local/airflow/dags/scripts/drugdevelopment/ingest_koios_raw/DaPlex_Ingestion_Config.json', 'r') as json_file:
    config_data = json.load(json_file)
...
def process_table(matching_items, last_modified_pst):
    ...
    for matching_item in matching_items:
        ...
        if delimiter == 'Excel' and matching_item.get('multi_sheet_tables'):
            # and len(matching_item['multi_sheet_tables']) >=1:
            ...
            # # Only process as a single-sheet file if multi_sheet_tables is not defined
            # if not matching_item.get('multi_sheet_tables'):
            processed_list.append({'table_name': table_name,
                                   'sheet_name': matching_item.get('sheet_name', None),
        ...
        for process_item in processed_list:
            ...
            logging.info(f"Loaded sheet: {process_item['sheet_name']} → table: {process_item['table_name']}")  
        #---------New Code Ends Here-----------------  
        file_processed_list.append(s3_key)
        
        logging.info(f"file processed for {s3_key} into table {table_name} for study id {study_id}")
