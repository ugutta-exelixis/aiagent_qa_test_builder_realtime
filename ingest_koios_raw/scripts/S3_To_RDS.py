...
import csv
from io import StringIO, BytesIO  #read and store data into in-memory
import csv
import sqlalchemy
import datetime
import re
...
import chardet
import pytz
import urllib.parse # to parse password containing special characters like @
import magic  # to detect the file_format and file_delimiter
# import os
...
class LoadS3ToRDS:
    ...
    def detect_file_format(self, file_content: bytes) -> str:
        ...
        try:
            # Use python-magic to detect MIME type from the file's binary content
            mime_type = magic.from_buffer(file_content, mime=True)
            logger.info(f"Detected MIME type by magic: {mime_type}")

            # If the file is identified as plain text, we try to determine if it's a CSV or a simple TXT
            if mime_type == "text/plain":
                # Detect file encoding using chardet to decode bytes into text safely
                encoding_info = chardet.detect(file_content)
                detected_encoding = encoding_info['encoding'] or 'utf-8'
                # Decode a sample portion of the file content using the detected (or fallback) encoding
                try:
                ...
                try:
                    # Try to detect a delimiter (e.g., comma, tab, pipe, semicolon) using a custom sniffer
    ...
    def load_csv_and_xlsx_to_dataframe(self, last_modified_pst,sheet_name,expected_columns):
        ...
        # We use BytesIO to create an in-memory binary stream so the S3 file can be read directly like a file object without saving it locally.
        ...
        # Since MIME types returned by python-magic differ slightly from actual file extensions,
        # we map them manually to the expected file extension format used in our workflow.
        ...
        if file_extension == 'xlsx':
            ...
            # Read sheet without header to detect it
            temp_df = pd.read_excel(xls, sheet_name=target_sheet, header=None, dtype=str)

            header_row_index = 0  # default
            if expected_columns:
                expected_lower = [col.lower() for col in expected_columns]
                for i, row in temp_df.iterrows():
                    # Normalize row values: replace \n with space, strip spaces
                    row_values = [str(col).replace('\n', ' ').strip().lower() for col in row.dropna().astype(str).tolist()]
                    # Count matches with expected columns
                    matches = [col for col in expected_columns if col in row_values]
                    if len(matches) >= max(1, len(expected_columns) // 2):  # threshold for match
                        header_row_index = i
                        break

            # Read sheet again with detected header
            df = pd.read_excel(xls, sheet_name=target_sheet, header=header_row_index)
            if df.columns[0].startswith('Unnamed'):
                df = df.iloc[:, 1:]

            # Normalize column names to remove \n
            df.columns = [str(col).replace('\n', ' ').strip() for col in df.columns]
            # newly added to convert datetime to date only
            datetime_cols = df.select_dtypes(include=['datetime64[ns]']).columns
            for col in datetime_cols:
                df[col] = df[col].dt.strftime('%d%b%Y') # Convert to ddmmmyyyy format (e.g., 21Nov2025)
        # if file_extension == 'xlsx':
        ...
        #     df = xls.parse(xls.sheet_names[0], header=None)
            ...
            df= pd.read_sas(s3_url,encoding='latin-1')
    ...
    def load_data_to_rds(self,secret_name,s3_key,table_name,delimiter=None,schema_name=None,last_modified_pst=None,study_id = None, sheet_name=None, expected_columns=None):
        self.key = s3_key
        ...
        try:
            df = self.load_csv_and_xlsx_to_dataframe(self.last_modified_pst,sheet_name,
                    expected_columns)
