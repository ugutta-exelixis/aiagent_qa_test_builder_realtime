...
,md5(study_protocol_number||COALESCE(phase,'phase')||COALESCE(cohort,'cohort')||country_name||month_end_date) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl495-101') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl495-101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl495-101') }})
union all
...
,md5(study_protocol_number||COALESCE(phase,'phase')||COALESCE(cohort,'cohort')||country_name||month_end_date) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb628-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb628-101') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb628-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb628-101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb628-101') }})
...
,REPLACE(upper(country_name), 'UNITED STATES OF AMERICA', 'UNITED STATES') AS country_name
,cast(month_end_date as varchar) as  month_end_date
,cast (planned_subjects as integer) as planned_subjects
,partition_date as load_date
,md5(study_protocol_number||COALESCE(phase,'phase')||COALESCE(cohort,'cohort')||country_name||month_end_date) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-311') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-311') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-311') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-311') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-311') }})
...
,md5(study_protocol_number||COALESCE(phase,'phase')||COALESCE(cohort,'cohort')||country_name||month_end_date) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb371-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb371-101') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb371-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb371-101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xb371-101') }})
...
,md5(study_protocol_number||COALESCE(phase,'phase')||COALESCE(cohort,'cohort')||country_name||month_end_date) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-202') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-202') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-202') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-202') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_proj_xl092-202') }})
