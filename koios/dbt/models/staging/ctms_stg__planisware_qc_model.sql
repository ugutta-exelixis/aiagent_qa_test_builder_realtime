...
select
  ...
  case when SPLIT_PART("activity type", '-', 1) ='LPLV' then 'CCO' else SPLIT_PART("activity type", '-', 1) end as activity_type_actual,
	--SPLIT_PART("activity type", '-', 1) as activity_type_actual,
...
where 
	last_modified_date = (select MAX(last_modified_date)from koios_raw.planisware_milestones)
	and partition_date = (select MAX(partition_date) from koios_raw.planisware_milestones)
      ),
    actual_counts AS (
  SELECT 
    protocol_number,
  ...
  and protocol_number not in (select distinct protocol_number from koios_staging.planisware_req_ref where state='blocked')
...
new_activities AS (
  ...
  left join koios_staging.planisware_req_ref pe
...
duplicate_activities AS (
  SELECT 
    ...
    'duplicate' AS status
  FROM actual_counts ac
  join koios_staging.planisware_req_ref pe
    ON ac.protocol_number = pe.protocol_number 
    AND ac.activity_type_actual = pe.milestones
    and ac.study_phase_norm= pe.study_phase
    --and pe.expected_count = ac.actual_count
...
missing_activities AS (
  SELECT 
    ...
    pe.expected_count,
    'missing' AS status
  FROM koios_staging.planisware_req_ref pe
  left join actual_counts ac
    ...
    and pe.protocol_number not in (select distinct protocol_number from koios_staging.planisware_req_ref where state='blocked')
    --and pe.protocol_number in ('XB010-101','XL092-304')
),
...
union all
  (select null as project, null as project_type, null as is_public_version, null as line_identifier, null as name, 
   null as activity_type,milestones,protocol_number,study_phase,null as cohort, 
   null as planned_date, null as approved_baseline,null as actual_date, null as original_baseline, 
  (select max(last_modified_date) from planisware_data) as last_modified_date,
  (select max(load_date) from planisware_data) as load_date,
   null as is_required,
   md5(protocol_number) as protocol_number_sk,null as protocol_no_study_phase_sk, null as protocol_activity_type_actual_sk,
   'ok' as status
   from koios_staging.planisware_req_ref where 
   state='blocked')
