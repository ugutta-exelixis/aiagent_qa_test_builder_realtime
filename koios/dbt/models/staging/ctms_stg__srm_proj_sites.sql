...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb002_101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb002_101') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092_001') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092_001') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092_002') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092_002') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092_303') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092_303') }})
...
FROM
   ...
   where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl102_101') }}))
   and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl102_101') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl114_101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl114_101') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_021') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_021') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_311') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_311') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_312') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_312') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_315') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl184_315') }})
...
,md5(study_protocol_number||country_name||month_end_date) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl495-101') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl495-101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl495-101') }})
...
FROM
    ...
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb628-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb628-101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb628-101') }})
...
FROM
    ...
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092-311') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092-311') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092-311') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb371-101') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xb371-101') }})
...
FROM
    ...
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092-202') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_proj_sites_xl092-202') }})


)

select * from ctms_stg__srm_proj_sites
