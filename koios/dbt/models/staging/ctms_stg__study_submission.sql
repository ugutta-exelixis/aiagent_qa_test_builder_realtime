...
,md5(study_protocol_number||study_country_id||coalesce(cast(study_site_id as varchar),'study_site_id')||submission_id) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_study_submission_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xl495-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xl495-101') }})
union all
...
,md5(study_protocol_number||study_country_id||coalesce(cast(study_site_id as varchar),'study_site_id')||submission_id) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_study_submission_xb628-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xb628-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xb628-101') }})
...
,md5(study_protocol_number||study_country_id||coalesce(cast(study_site_id as varchar),'study_site_id')||submission_id) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_study_submission_xl092-311') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xl092-311') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xl092-311') }})
...
,null as planned_approval_date
,null as renewal_report_due_date
,null as renewal_report_complete_date
,null as comments
,partition_date as load_date
,md5(study_protocol_number||study_country_id||coalesce(cast(study_site_id as varchar),'study_site_id')||submission_id) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_study_submission_xb371-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xb371-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xb371-101') }})
...
,md5(study_protocol_number||study_country_id||coalesce(cast(study_site_id as varchar),'study_site_id')||submission_id) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_study_submission_xl092-202') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xl092-202') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_study_submission_xl092-202') }})
