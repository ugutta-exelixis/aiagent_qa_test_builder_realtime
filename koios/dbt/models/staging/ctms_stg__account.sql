...
union all 
select distinct account_source_id as account_id
,account_name as name
,partition_date as load_date
,md5(account_source_id) as hash_key
from
    {{ source ('koios_raw', 'q_exe_account') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'q_exe_account') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'q_exe_account') }})
