...
,last_modified_date
from
    {{ source ('koios_raw', 'xb010-101_query_detail') }}
    where last_modified_date = (select max(last_modified_date) from {{source ('koios_raw', 'xb010-101_query_detail') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xb010-101_query_detail') }})
...
,last_modified_date
from
    {{ source ('koios_raw', 'xl495-101_query_detail') }}
    where last_modified_date = (select max(last_modified_date) from {{source ('koios_raw', 'xl495-101_query_detail') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xl495-101_query_detail') }})
