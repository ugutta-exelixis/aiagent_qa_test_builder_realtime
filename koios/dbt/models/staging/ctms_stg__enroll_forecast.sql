...
,md5("study_protocol_number"||COALESCE(CAST("phase" AS VARCHAR),'PHASE')||coalesce(CAST("cohort" AS VARCHAR),'COHORT')||"country_name"||"month_end_date") as HASH_KEY
FROM
    {{ source ('koios_raw', 'enroll_forecast_xl092-001') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'enroll_forecast_xl092-001') }})
    and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'enroll_forecast_xl092-001') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'enroll_forecast_xl092-001') }}))
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'enroll_forecast_xl092-001') }})
...
,md5("study_protocol_number"||COALESCE(CAST("phase" AS VARCHAR),'PHASE')||coalesce(CAST("cohort" AS VARCHAR),'COHORT')||"country_name"||"month_end_date") as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl495-101') }})
   and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl495-101') }}
   where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl495-101') }}))
   and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl495-101') }})
...
,md5("study_protocol_number"||COALESCE(CAST("phase" AS VARCHAR),'PHASE')||coalesce(CAST("cohort" AS VARCHAR),'COHORT')||"country_name"||"month_end_date") as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb010-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb010-101') }})
   and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb010-101') }}
   where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb010-101') }}))
   and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb010-101') }})
...
FROM
   ...
   and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb628-101') }}
   where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xb628-101') }}))
...
FROM
   ...
   and cast(month_end_date as date) = (select max(cast(month_end_date as date)) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl092-311') }}
   where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_srm_enroll_forecast_xl092-311') }}))
