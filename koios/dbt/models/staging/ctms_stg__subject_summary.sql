...
,Null as "study stage"
,cast("date of first icf" as varchar) as "date of first icf"
,"rand or screen fail date" as "enroll or screen fail date"
,Null as cohort_desc
,Null as "cohort"
,"treatment arm"
,null as tumor_type
...
,Null as dose_level_treatment_arm
,cast("most recent visit" as varchar) as "most recent visit"
,"most recent date of visit" as "most recent date of visit"
,"eot date (cabo)" as "eotxb date"
,cast("death date" as varchar) as "death date"
,"eorfup date"
,"eos date"
,"eos reason"
...
,null as "esfu reason"
,cast("total expected pgs" as int) as "total expected pgs"
,cast(nullif(regexp_replace("missing pgs (entry not started)", '[^0-9]', '', 'g'), '') AS INTEGER) as "missing pgs (entry not started)"
,null as "% missing pgs (entry not started)"
,cast("missing <30 days" as varchar) as "missing <30 days"
...
,cast("31-60 days after report date" as varchar) as "31-60 days after report date"
,cast("61-90 days after report date" as varchar) as "61-90 days after report date"
,cast("91-120 days after report date"as varchar) as "90-180 days after report date"
,cast("studyid" as varchar) as "studyid"
,partition_date as load_date
,last_modified_date as EDC_Data_Refreshed_Date
from 
    {{ source ('koios_raw', 'xl184-312_subject_summary') }} 
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'xl184-312_subject_summary') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xl184-312_subject_summary') }})
...
,Null as "study stage"
,cast("date of first icf" as varchar) as "date of first icf"
,"randomization or screen fail date" as "enroll or screen fail date"
,Null as cohort_desc
,Null as "cohort"
,null as treatment_arm
,null as tumor_type
...
,cast("most recent visit" as varchar) as "most recent visit"
,"most recent date of visit" as "most recent date of visit"
,"eot date (blinded)" as "eotxb date"
,cast("death date" as varchar) as "death date"
,"eorfup date"
,"eos date"
,"eos reason"
...
,last_modified_date as EDC_Data_Refreshed_Date
from 
    {{ source ('koios_raw','xl184_311_subject_summary') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'xl184_311_subject_summary') }} )
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xl184_311_subject_summary') }} )
...
,cast("subject status" as varchar) as "subject status"
,case
    when coalesce("study stage", "Study Phase") in ('Dose Expansion','Expansion')  then 'Expansion'
    -- when "study stage" is null and  "date of first icf"::date >= '18-Aug-25'  then 'Expansion'
    when coalesce("study stage", "Study Phase") in ('Dose Escalation','Escalation') then 'Escalation'
    else  coalesce("study stage", "Study Phase")
end as "study stage"
,cast("date of first icf" as varchar) as "date of first icf"
,cast("enrolled/randomized/ screen fail date" as varchar) as "enroll or screen fail date"
,case when coalesce(a.cohort,b.cohort) in('Not available','Screen Fail') then null 
else coalesce(a.cohort,b.cohort)
end as cohort_desc
,case
    when split_part(a.cohort,':',1) like 'COHORT A1%' then 'Cohort A1'
    when split_part(a.cohort,':',1) like 'COHORT A2%' then 'Cohort A2'
    else initcap(split_part(a.cohort,':',1))
end as cohort
,cast("treatment arm" as varchar) as treatment_arm
,case 
    when coalesce(a.cohort,b.cohort) is null and coalesce("study stage", "Study Phase") not in('Dose Expansion','Expansion') then 'SOLID TUMORS' 
    else trim(split_part(a.cohort,':',2))
end as tumor_type
,Null as race
,Null as ethnic
,cast(split_part("treatment arm",' ',1) || '_' ||split_part("treatment arm",' ',2) || ' MG/KG' as varchar) as dose_level_treatment_arm
-- ,cast("treatment arm" as varchar) as treatment_arm
,cast("most recent visit" as varchar) as "most recent visit"
,cast("most recent date of visit" as varchar) as "most recent date of visit"
,null as "eotxb date"
...
,cast("studyid" as varchar) as "studyid"
,partition_date as load_date
,a.last_modified_date as EDC_Data_Refreshed_Date
from
    {{ source ('koios_raw', 'xb010-101_subject_summary') }} a
    left join {{source('koios_raw','xb010-101_study_phase_ref')}}  b
    on split_part(a."subject number",'-',3) = b.subject
    where a.last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'xb010-101_subject_summary') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xb010-101_subject_summary') }})
...
,cast("study stage" as varchar) as "study stage"
,cast("date of first icf" as varchar) as "date of first icf"
,cast("enroll/rand|or screen fail|date" as varchar) as "enroll or screen fail date"
,null as cohort_desc
,split_part(cohort,':', 1) as cohort
...
,Null as race
,Null as ethnic
,cast(split_part("treatment description arm",' ',1) || '_' ||split_part("treatment description arm",' ',2) || ' MG/KG' as varchar) as dose_level_treatment_arm
-- ,cast("treatment arm" as varchar) as treatment_arm
,cast("most recent visit" as varchar) as "most recent visit"
,cast("most recent date of visit" as varchar) as "most recent date of visit"
,null as "eotxb date"
...
,last_modified_date as EDC_Data_Refreshed_Date
from
    {{ source ('koios_raw', 'xl495-101_subject_summary') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'xl495-101_subject_summary') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xl495-101_subject_summary') }})
