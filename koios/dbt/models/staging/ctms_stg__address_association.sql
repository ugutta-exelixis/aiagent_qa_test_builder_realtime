...
,md5(study_protocol_number||address_id||associated_record_id||type) as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_address_association_xl495-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_address_association_xl495-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_address_association_xl495-101') }})
union all 
select 
clinical_study_source_id as study_protocol_number
,cast(address_source_id as varchar) as address_id
,'STUDY_SITE' as associated_dataset
,cast(study_site_source_id as varchar) as associated_record_id
,address_type as type
,partition_date as load_date
,md5(clinical_study_source_id||address_source_id||study_site_source_id||'') as HASH_KEY
FROM
    {{ source ('koios_raw', 'q_exe_siteaddress_xl092-304') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'q_exe_siteaddress_xl092-304') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'q_exe_siteaddress_xl092-304') }})
...
,md5(clinical_study_source_id||address_source_id||study_site_source_id||'') as HASH_KEY
FROM
    {{ source ('koios_raw', 'q_exe_siteaddress_xl092-305') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'q_exe_siteaddress_xl092-305') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'q_exe_siteaddress_xl092-305') }})
...
,md5(clinical_study_source_id||address_source_id||study_site_source_id||'') as HASH_KEY
FROM
    {{ source ('koios_raw', 'q_exe_siteaddress_xl092-009') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'q_exe_siteaddress_xl092-009') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'q_exe_siteaddress_xl092-009') }})
union all
...
,md5(study_protocol_number||address_id||associated_record_id||type) as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_address_association_xb628-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_address_association_xb628-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_address_association_xb628-101') }})
...
,md5(study_protocol_number||address_id||associated_record_id||type) as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_address_association_xl092-311') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_address_association_xl092-311') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_address_association_xl092-311') }})
...
,md5(study_protocol_number||address_id||associated_record_id||type) as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_address_association_xb371-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_address_association_xb371-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_address_association_xb371-101') }})
...
,md5(study_protocol_number||address_id||associated_record_id||type) as HASH_KEY
FROM
    {{ source ('koios_raw', 'pra_exl_address_association_xl092-202') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_address_association_xl092-202') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_address_association_xl092-202') }})
