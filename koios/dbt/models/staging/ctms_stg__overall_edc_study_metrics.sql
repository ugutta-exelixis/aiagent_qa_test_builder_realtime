...
select 
    ...
    to_date(partition_date,'YYYY-MM-DD')load_date
from
    {{ source ('koios_raw', 'xb010-101_overall_edc_study_metrics') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'xb010-101_overall_edc_study_metrics') }})
    and country LIKE '%Subtotal'AND country != 'Subtotal'
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xb010-101_overall_edc_study_metrics') }})
...
select 
    ...
    to_date(partition_date,'YYYY-MM-DD')load_date
from
    {{ source ('koios_raw', 'xl495-101_overall_edc_study_metrics') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'xl495-101_overall_edc_study_metrics') }})
    and country LIKE '%Subtotal'AND country != 'Subtotal'
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'xl495-101_overall_edc_study_metrics') }})
