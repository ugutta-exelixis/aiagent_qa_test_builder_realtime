with subject_visit_summary_report as(
   ...
   case when study_id = 'XL092-304' then to_date("actual date",'DD Mon YYY') 
    ...
    case 
        when study_id in ('XL309-101') THEN MD5(RIGHT(subject, 6))
        ELSE MD5(RIGHT(subject, 4)) 
    END as participant_id_sk,
    case 
        when study_id in ('XL309-101') THEN MD5(RIGHT(subject, 6))
        ELSE MD5(RIGHT(subject, 4)) 
    END as participant_number_sk,
    case 
        when study_id in ('XL309-101') THEN RIGHT(subject,6)
        ELSE RIGHT(subject,4)
    END as participant_number,
     case 
        when study_id in ('XL092-009','XL092-305','XB010-101', 'XL495-101','XL309-101','XB628-101','XB371-101','XL092-201','XL092-011') then studysite
        when study_id not in ('XL092-009','XL092-305','XB010-101', 'XL495-101') and length(subject) >= 9 then split_part(subject,'-',1) 
        when study_id not in ('XL092-009','XL092-305','XB010-101', 'XL495-101') and length(subject) < 5 then right(studysite,4) 
    else null 
    end as site_id,
...
subject_summary_by_visit as(
    ...
    case when studyid in ('XL092-009','XL092-305','XB010-101', 'XL495-101') 
...
site_info as (
    ...
    ,md5(concat(study_id||site_number)) as study_site_id_sk
    from  {{ref ('ctms_stg__study_site_phase')}}
)

select distinct fpvf.* from fct_participant_visit_final fpvf
inner join site_info si
on fpvf.study_id_sk = si.study_id_sk
and fpvf.study_site_id_sk = si.study_site_id_sk
-- and dss.site_type = 'Main'
