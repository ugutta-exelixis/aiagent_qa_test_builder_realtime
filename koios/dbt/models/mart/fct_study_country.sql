...
select distinct
   ...
   from {{ref ('ctms_stg__study_site')}}
    where study_site_status in ('Activated', 'Active Enrolling', 'Enrollment Open','Enrollment Closed','Closed','Close Out Ready','Enrollment On-Hold') and site_type = 'Main'
...
select distinct
    ...
    group by study_id,study_country_id_sk,
    study_id_sk,
    country_status
    )
