...
Pi_Contact as (
	...
	from (
		...
		and START_DATE not in ('00-000-0000')
...
dim_study_site as(
	...
	dim_study_site_phase as (
		...
		--case when ss.study_id= 'XL309-101' and ss.site_number = '119' then '108'
		--else ss.site_number
		parent_site_number,
		ss.site_number,
		case when ss.study_id= 'XL309-101' and ss.site_number <> parent_site_number then 'Satellite'
		else 'Main' 
		end as site_type,
		...
		ss.site_name,
		ss.pi_name as pi_name,
		sc."pi_phone" as pi_phone_number,
		sc."pi_email" as pi_email,
		sc."sc_name" as sc_name,
		sc."sc_phone" as sc_phone_number,
		sc."sc_email" as sc_email
		from 
		{{ref ('ctms_stg__study_site_phase')}} ss
		left join 
		{{ref ('ctms_stg__site_contact_details_phase')}} sc
