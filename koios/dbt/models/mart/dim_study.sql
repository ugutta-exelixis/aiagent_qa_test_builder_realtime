...
dim_study as(
    select
        cps.study_id_sk,
        cps.study_id,
        coalesce(sd.study_title,cps.study_title) as study_title,
        -- case 
        --     when cps.study_id in ('XL092-009','XL309-101') then cps.study_title
        --     else sd.study_title
        -- end as study_title,
        coalesce(sd.study_drug, cps.study_drug ) as study_drug,
        -- case 
        --     when cps.study_id in ('XL092-009','XL309-101') then cps.study_drug
        --     else sd.study_drug
        -- end as study_drug,
        -- case 
        --     when cps.study_id in ('XL092-009','XL309-101') then cps.Phase
        --     else sd.phase
        -- end as phase,
        coalesce(cps.phase, sd.phase ) as phase,
        -- case 
        --     when cps.study_id in ('XL092-009','XL309-101') then initcap(cps.indication)
        --     else initcap(sd.indication)
        -- end as indication,
        coalesce(cps.indication,sd.indication) as Indication,
