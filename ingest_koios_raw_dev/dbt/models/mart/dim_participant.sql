...
when status = 'Screen Failure' and "tumor type" <> 'Solid Tumor' then 'Expansion'
...
cohort as (
    select
        ...
        status  
    from {{ref ('ctms_stg__subject_summary_report')}}
    where study_id in ('XB002-101','XL092-002','XL092-009')
...
subject_summary_report as(
    select distinct
    RIGHT(subject,4) AS participant_id_irt,
    RIGHT(subject,4) as participant_number_irt,
    md5(RIGHT(subject,4))as participant_id_sk_irt,
    md5(study_id || RIGHT(subject,4)) as subject_summary_report_sk_irt,
...
dim_participant_final as (
    select distinct participant_id,participant_id_sk,study_subject_sk,participant_number,study_id,study_id_sk
,max(race) as race
...
dim_participant_cohort as (
    ...
    ,dpf.participant_status_reason
    ,dpf.study_phase
    ,case when dpf.study_id = 'XB002-101' and dpf.study_phase = 'Escalation' then 'SOLID TUMORS' else  c."tumor type" end as tumor_type
    ,c.cohort_desc
    ,case when dpf.study_id = 'XB002-101' and dpf.study_phase = 'Escalation' then 'COHORT A' else c.cohort end as cohort
    ,c.treatment_arm
...
dim_participant_cohort_map as (
	...
	union
	select distinct d.*,null as cohort_map  from dim_participant_cohort d
	where d.cohort is null and study_id not in ('XL092-002','XB002-101','XL092-009')
	union
...
select distinct dpf.participant_id
    ...
    when dpf.study_id in ('XB002-101', 'XL092-002','XL092-009') then cs."cohort status"
