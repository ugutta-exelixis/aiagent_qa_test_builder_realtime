...
last_modified_date as IRT_Data_Refreshed_Date
FROM    
{{ source ('koios_raw', 'subject_summary_report_xb010_101') }}    
where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'subject_summary_report_xb010_101') }})    
