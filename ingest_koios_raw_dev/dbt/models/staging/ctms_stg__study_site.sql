...
,md5(study_protocol_number||study_country_id||study_site_id||account_id||pi_contact_id) as hash_key
FROM
    {{ source ('koios_raw', 'pra_exl_study_site_xb010-101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'pra_exl_study_site_xb010-101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'pra_exl_study_site_xb010-101') }})
