...
,case 
	...
	when b.visit = 'Subject Status Change' then "subject status"
	else b.visit 
end as  "visit description"
,cast(b."expected visit date" as varchar) as  "scheduled date"
,cast(to_char(TO_DATE(a."visit date",'DD-MON-YYYY'),'DD MON YY')as varchar) as  "actual date"
...
FROM
    {{ source ('koios_raw', 'subject_visit_summary_report_xb010_101') }}
    where last_modified_date = (select max(last_modified_date) from {{ source ('koios_raw', 'subject_visit_summary_report_xb010_101') }})
    and partition_date = (select max(partition_date) from {{ source ('koios_raw', 'subject_visit_summary_report_xb010_101') }})
